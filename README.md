...
**Liveness and readiness are separate endpoints, and the split is the interesting part.** `/healthz` deliberately checks nothing but itself, because a failed liveness probe *kills* the container — wiring a dependency check to it would turn a brief Postgres outage into a fleet-wide restart, every pod dying at once and reconnecting together at the worst possible moment. `/readyz` pings Postgres, since a failed readiness probe only withdraws the pod from Service endpoints.

Readiness checks Postgres and nothing else, on purpose. Postgres is the only dependency whose loss leaves either binary unable to do anything at all. A worker that loses Kafka still claims through the reconciliation sweep; one that loses Redis still executes, because the rate limiter fails open; a scheduler that loses etcd simply stays a follower, which is a correct state rather than a broken one. Marking any of those "unready" would withdraw a pod that is still getting work done.

Verified by breaking it rather than by reading it: with Postgres frozen (`docker pause`), `/readyz` returns `503 not ready: context deadline exceeded` while `/healthz` stays `200` — so Kubernetes pulls the pod from endpoints without killing it — and readiness returns to `200` on unpause with no restart.
